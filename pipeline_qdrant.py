...
class Pipeline:
    ...
    def _generate_tags(self, dims: List[str], dataset: str = None, chunks_file: str = None, docs_source: str = None) -> Dict[str, Dict[str, List[str]]]:
        ...
        for doc_id, tag_map in list(results.items()):
            if doc_id in chunk_text_map and isinstance(tag_map, dict):
                results[doc_id] = self._filter_dimension_tags(chunk_text_map[doc_id], tag_map)
        ...
        print(f"    待抽取 {total_pending} 条，批量大小={batch_size}（每批一次 LLM 请求）")

        for start in range(0, total_pending, batch_size):
            batch = pending[start:start + batch_size]
            try:
                extracted_batch = miner.extract_multi_chunk_dimensions(batch, dims)
            except Exception as e:
            ...
            for chunk in batch:
                ...
                results[doc_id] = self._filter_dimension_tags(
                    chunk["doc_text"], extracted_batch.get(doc_id, {})
                )
    ...
    @staticmethod
    def _filter_dimension_tags(text: str, tag_map: Dict[str, Any]) -> Dict[str, List[str]]:
            ...
            for value in values:
                ...
                if len(chars) >= 3:
                    ...
                    if overlap >= 0.75:
                        kept.append(value)
            if kept:
                filtered[str(dim).strip()] = list(dict.fromkeys(kept))
        return filtered
