...
def _call_llm(api_key: str, prompt: str) -> dict[str, Any]:
    compat_key = os.environ.get("LLM_API_KEY", "")
    compat_enabled = os.environ.get("LLM_OPENAI_COMPAT", "0").lower() in {"1", "true", "yes"}
    if compat_enabled or (not os.environ.get("DASHSCOPE_API_KEY", "") and compat_key):
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument("--start-index", type=int, default=0, help="候选列表起始下标，用于并行分片复核")
    parser.add_argument("--end-index", type=int, default=0, help="候选列表结束下标（不含），0 表示到末尾")
...
def main() -> int:
    ...
    end_index = args.end_index if args.end_index > 0 else len(all_candidates)
    if start_index > end_index:
        raise ValueError("start-index 不能大于 end-index")
    candidates = all_candidates[start_index:end_index]
    if args.max_items > 0:
