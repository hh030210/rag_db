...
def _compact_result(result: dict[str, Any], rank: int, trace_chars: int) -> dict[str, Any]:
    ...
    # 保留维度融合证据，便于复核本轮实验是否真正使用了 Qdrant dim_* 字段。
    for key in (
        "dim_name", "tag_name", "recall_method", "rerank_score", "internal_fusion",
        "spot_name", "entity_score", "dimension_coverage", "matched_dimensions",
        "match_sources", "match_source",
    ):
        if key in result and result.get(key) not in (None, ""):
            compact[key] = result.get(key)
    if result.get("tag_hits"):
        compact["tag_hits"] = result.get("tag_hits")[:8]
    return compact
...
def _config_snapshot(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "model": qa.QA_MODEL if getattr(qa, "QA_OPENAI_COMPAT", False) or not qa.DS_API_KEY else qa.DS_MODEL,
...
def _run_one(case: dict[str, Any], args: argparse.Namespace, expander: Any = None) -> dict[str, Any]:
    ...
    record = {
        ...
        "qa": {
            ...
            "context_stats": qa_result.get("context_stats", {}),
        },
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--context-chars",
        type=int,
        default=qa.DEFAULT_QA_CONTEXT_CHARS,
        help="每个检索 chunk 传给问答模型的字符数，默认 0 表示使用完整 chunk；显式限长时保留相关窗口",
    )
