...
for i in range(0, len(all_texts), BATCH):
    ...
    pct = (i + BATCH) * 100 // len(all_texts)
    elapsed = time.time() - t0
    remaining = elapsed * (len(all_texts) - i - BATCH) / max(1, i + BATCH)
    print(f"  {pct}% ({i+BATCH}/{len(all_texts)}), ETA {remaining:.0f}s")
del chunks, all_texts
embed_time = time.time() - t0
print(f"Embeddings done in {embed_time:.1f}s ({len(all_embs)*60//max(1,int(embed_time)):.0f}/min)")
