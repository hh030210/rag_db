...
class BaseEvaluator(ABC):
    def __init__(self, task: BaseTask, model: BaseLLM, retriever: BaseRetriever,
        ...
        # 文件名中不能包含 '/', 用 '_' 替换，避免 Windows 下创建文件失败
        safe_model_name = model.__class__.__name__
        output_dir = os.path.join(output_dir, f'{collection_name}_top{similarity_top_k}_{safe_model_name}')

        if not (os.path.exists(output_dir) and os.path.isdir(output_dir)):
            os.makedirs(output_dir)
        # model_name 中可能含 '/', 替换为 '_' 避免 Windows 路径错误
        safe_params_name = model.params["model_name"].replace('/', '_').replace('\\', '_')
        self.output_path = os.path.join(
            output_dir, f'{self.task.__class__.__name__}_{safe_params_name}.json'
        )
