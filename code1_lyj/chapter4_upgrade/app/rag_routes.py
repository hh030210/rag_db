...
async def get_collections():
    ...
    if client:
        try:
            # 使用try-except包装ChromaDB调用，处理版本兼容性问题
            try:
                chroma_collections = client.list_collections()
                # 确保返回的是列表
                if not isinstance(chroma_collections, list):
                    chroma_collections = list(chroma_collections) if chroma_collections else []
            except Exception as e:
                logger.warning(f"ChromaDB list_collections 失败: {str(e)}")
                chroma_collections = []
