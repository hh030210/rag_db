"""
Tourist RAG演示系统 - 集成到Chapter4的新功能
包含数据集展示、Prompt管理、聚类展示、答案生成等功能
"""
import os
import sys
import json
...
class TouristRAGPage:
    ...
    def create_dataset_tab(self):
        ...
        def load_dataset():
            ...
            dataset_list = []
            for item in eval_data[:50]:  # 只显示前50条
                ...
                dataset_list.append({
                    "问题ID": question_id[:20] + "..." if len(question_id) > 20 else question_id,
                    "景点": attraction[:20] if attraction else "未知",
                    "问题": question[:40] + "..." if len(question) > 40 else question,
                    "参考答案": answer[:50] + "..." if len(answer) > 50 else answer,
                    "参考来源数": len(source),
                    "已有Prompt": has_prompt,
                    "完整数据": item
                })
        ...
        def on_select(evt: gr.SelectData):
            ...
            if evt.index is not None:
                ...
                if evt.index[0] < len(dataset_list):
                    ...
                    full_data = row_data.get("完整数据", {})
                    return (
                        full_data.get("question", ""),
                        full_data.get("answer", ""),
                        "\n\n".join(full_data.get("source", [])) if full_data.get("source") else "无",
                        full_data.get("question_id", "")
                    )
            return "", "", "", ""
        
        with gr.Row():
            with gr.Column(scale=2):
                gr.Markdown("### 📊 数据集概览 (显示前50条)")
        ...
        # 页面加载完成后自动刷新数据（使用gr.load事件）
        def on_page_load():
            return load_dataset()
        
        # 注意：不在初始化时加载数据，避免Gradio 6.0的effect循环问题
        # 用户需要点击刷新按钮加载数据
    
    # ==================== 页面2: Prompt初始化展示 ====================
    
    def create_prompt_init_tab(self):
        """创建Prompt初始化展示标签页"""
        ...
        def load_prompts():
            ...
            for question_id, data in prompts.items():
                ...
                prompt_list.append({
                    ...
                    "场景类型": scene_analysis.get('scene_type', '未知')[:15],
        ...
        def on_select(evt: gr.SelectData):
            ...
            if evt.index is not None:
                ...
                if evt.index[0] < len(prompt_list):
                    ...
                    return (
                        full_data.get("question", ""),
                        full_data.get("ground_truth", ""),
                        "\n".join([f"- {k}" for k in full_data.get("key_aspects", [])]),
                        full_data.get("scene_analysis", {}).get("scene_type", "未知"),
                        full_data.get("scene_analysis", {}).get("complexity", "未知"),
                        full_data.get("full_prompt", "")
                    )
            return "", "", "", "", "", ""
        ...
        def load_clusters():
            ...
            for cluster_id, prompts in cluster_prompts.items():
                ...
                cluster_list.append({
                    ...
                    "完整数据": {"cluster_id": cluster_id, "questions": questions, "prompts": prompts}
                })
...
"""
    ...
    def create_query_match_tab(self):
        """创建Query匹配聚类标签页"""
...
"""
    ...
    def create_answer_generation_tab(self):
        """创建答案生成与融合标签页"""
...
{answers[1]['answer']}""" if len(answers) > 1 else "未生成"
    ...
    def create_ui(self) -> gr.Blocks:
        """创建Tourist RAG系统的Gradio界面"""
        
        with gr.Blocks(title="Tourist RAG演示系统") as tourist_app:
            gr.Markdown("""
            # 🏛️ Tourist RAG演示系统
            
            基于群智优化的旅游问答RAG系统演示。使用现有的tourist向量库和优化后的Prompt。
            """)
            
            with gr.Tab("📊 数据集展示"):
                self.create_dataset_tab()
            
            with gr.Tab("📝 Prompt初始化"):
                self.create_prompt_init_tab()
            ...
            with gr.Tab("🔍 Query匹配聚类"):
                self.create_query_match_tab()
            
            with gr.Tab("🤖 答案生成与融合"):
                self.create_answer_generation_tab()
